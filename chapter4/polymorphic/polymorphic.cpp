...
public:
    ...
    explicit num_sequence(int beg_pos = 1, int len = 1, ns_type nst = ns_unset) {
    ...
    // 设置数列类别
    // 主要设置当前数列的_pmf、_isa、_elem等data member
    ...
    // 为了让外界查询此对象目前究竟代表何种数列，返回一个表示数列类型的字符串。
    const char *what_am_i() const {
        static char const *names[cnt_seq] = {
            "not set",
            "fibonacci",
            "pell",
            "lucas",
...
private:
    ...
    vector<unsigned int> *_elem{};  // 指向目前使用的vector
